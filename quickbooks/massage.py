...
def get_line_account(line_dict, qbbo, name=False):
    ...
    if qbbo in ["JournalEntry"]:
        ...
        account_id = line_dict["JournalEntryLineDetail"]["AccountRef"]["value"]

        account_name = line_dict["JournalEntryLineDetail"]["AccountRef"]["name"]

    elif qbbo in ["Bill", "Purchase"]:

        account_id = line_dict["AccountBasedExpenseLineDetail"]["AccountRef"]["value"]

        account_name = line_dict["AccountBasedExpenseLineDetail"]["AccountRef"]["name"]

    else:
        pp = pprint.PrettyPrinter(indent=1)
        pp.pprint(line_dict)
        raise Exception("Tell me how to find the account in a %s object!" % qbbo)
    if name:
        return account_name
    else:
        return account_id
...
def find_by_name(qbbo_list, name, qbs):
    ...
    if len(hit_list) == 0:
        ...
        raise Exception("More than one thing is named %s: %s." % (name, hit_list))
...
def line_entity(line_dict, qbbo, name=False):
    ...
    if qbbo in ["JournalEntry"]:
        id = line_dict["AccountBasedExpenseLineDetail"]["Entity"]["EntityRef"]["value"]
        entity_name = line_dict["AccountBasedExpenseLineDetail"]["Entity"]["EntityRef"]["name"]
    else:
        raise Exception("Tell me how to find the account in a %s object!" % qbbo)
...
def sure_id(qbo_session, qbbo, name_or_id, want='Id'):
    ...
    if name_match is not None and id_match is not None and not name_match == id_match:
        raise Exception("A different %s than Id %s has name %s!" % (qbbo, id_match, name_match))
        ...
        for name_dict in vce_list:
            ...
            for h in header_list:
                ...
                if h == "qbbo":
                    ...
                    elif h in ["SalesTermRef", "PaymentMethodRef", "TermRef", "ParentRef"]:
...
def ledgerize(transaction, qbo_session=None, headers=False, debug=False):
    ...
    id = rod["Id"]
    txn_date = datetime.strptime(rod["TxnDate"], "%Y-%m-%d")
    create_time = rod["MetaData"]["CreateTime"]
    last_updated_time = rod["MetaData"]["LastUpdatedTime"]
    sync_token = rod["SyncToken"]
    domain = rod["domain"]
    description = ""  # unless it's a JE, I THINK...
    department_ref = "Not implemented yet!"
    ...
    if "PrivateNote" in rod:
        private_note = rod["PrivateNote"]
    else:
        private_note = ""

    if "DocNumber" in rod:
        doc_number = rod["DocNumber"]
    else:
        doc_number = ""
    ...
    if "LinkedTxn" in rod:
        ...
        for lt in rod["LinkedTxn"]:
            lt_doc = lt["TxnType"]
            lt_num = lt["TxnId"]
            linked_txn.append(lt_doc + "/" + lt_num)

        linked_txn = "; ".join(linked_txn)
    ...
    if qbbo == "Bill":
        ...
        if "BankAccountRef" not in rod["CheckPayment"]:
            ...
            head_account = rod["CheckPayment"]["BankAccountRef"]["name"]
        entity_ref = rod["VendorRef"]["name"]
        polarity = "Credit"
        ...
        entity_ref = rod["CustomerRef"]["name"]
        polarity = "Debit"
        ...
        entity_ref = name_dict["FullyQualifiedName"]
        ...
        raise NotImplementedError("Implement QuickBooks.ledgerize()" + " for %s objects!" % qbbo, "e.g:", "%s" % id)
    ...
    if qbbo not in ["JournalEntry"]:
        ...
        ledger_lines_list.append([

            company_id, qbbo, id, line_number,
            domain, entry_subtype, sync_token,
            doc_number, txn_date, create_time, last_updated_time,
            private_note,
            head_account, currencyref, amount,
            description, entity_ref,
            department_ref, classref,
            linked_txn
    ...
    if len(rod["Line"]) < 1:
        ...
        if qbbo == "BillPayment":
            ...
            raise Exception("What should ledgerize do with a %s that has no split lines?" % qbbo)
    ...
    for split_line in rod["Line"]:
        ...
        if "LinkedTxn" in split_line:
            ...
            for lt in split_line["LinkedTxn"]:
                ...
                lt_num = lt["TxnId"]
                try:
                    lt_qbbo = qbrefs.linked_txn_correction[lt_doc]
                except KeyError:
                    print "Add %s to reference.linked_txn_correction! For now, just using that name." % lt_doc
        ...
        if qbbo == "Bill":
            ...
            if lt_qbbo == "Bill":
                ...
                for line in je_dict["Line"]:
                    ...
                    if line_account_id == qbo_session.get_ap_account():
                        ...
                        if lt_qbbo in ["JournalEntry"]:
                            line_polarity = line["JournalEntryLineDetail"]["PostingType"]

                        elif lt_qbbo in ["Purchase"]:

                            if "Credit" in je_dict and je_dict["Credit"] == "False":
                            ...
                            raise Exception("How can line_polarity == %s!?" % line_polarity)
                ...
                print split_line
                raise Exception("BillPayment %s is linked to a %s. What do I do?" % (id, lt_doc))
            ...
            item = split_line["SalesItemLineDetail"]["ItemRef"]["name"]

            if qbo_session is None:
                line_account = "Look up account for Item %s!" % item

            else:

                item_id = split_line["SalesItemLineDetail"]["ItemRef"]["value"]

                items = qbo_session.get_objects("Item")

                # print "Looking for account in Item[%s]." % item_id
                account_id = items[item_id]["IncomeAccountRef"]["value"]

                account_object = qbo_session.get_objects("Account")[account_id]
            ...
            if "Entity" in je_deets:
                entity_ref = je_deets["Entity"]["EntityRef"]["name"]
            else:
                entity_ref = ""
            ...
            line_account = split_line["AccountBasedExpenseLineDetail"]["AccountRef"]["name"]
            polarity = "Debit"
...
def ledger_lines(qbo_session, qbbo=None, id=None, line_number=None, headers=False, **kwargs):
    """
    ...
    if qbbo is None and id is None and line_number is None:
        ...
        if not hasattr(qbo_session, "ledger_lines_dict"):
            ...
            date_ordered_k_list = sorted(ledger_lines_dict.iterkeys(), key=lambda k: ledger_lines_dict[k][0])
        ...
        if id is not None:
            if qbbo is None:
                raise Exception("Can't give an Id (%s) and no object type." % id)
        ...
        for t in transactions:
            ...
            if line_number is None:
                ...
                if qbbo is None or id is None:
                    raise Exception("Can't give a line_number (%s) and not also a qbbo and an Id." % line_number)
    ...
    if qbbo == "JournalEntry":
        ...
        for ll in lls:
            ...
            if account_id is None:
                raise Exception("%s is not an existing account" % ll.line_account)

            detail_dict['AccountRef'] = {"name": ll.line_account, "value": account_id}

            if ll.EntityRef is not None:
                ...
                entity_type, entity_id = find_by_name(qbbo_list, entity_name, qbs)

                if entity_id is None:
                    if not auto_create_vcs:

                        error_message = "No Customer, Vendor, or Employee called %s exists." % ll.EntityRef
                        ...
                        account_classification = qbs.get_objects("Account")[account_id]["Classification"]
                        ...
                        new_entity_dict = fabricate(entity_type, entity_name, qbs)

                        entity_id = new_entity_dict["Id"]

                detail_dict["Entity"] = {"Type": entity_type, "EntityRef": {"name": entity_name, "value": entity_id}}

            if ll.DepartmentRef is not None:
                dept_id = find_by_name("Department", ll.DepartmentRef)

                detail_dict["DepartmentRef"] = {
                    "name": ll.Department,
                    "value": dept_id
                }

            if ll.ClassRef is not None:
                class_id = find_by_name("Class", ll.ClassRef)

                detail_dict["ClassRef"] = {
                    "name": ll.Department,
                    "value": class_id
                }
        ...
        raise Exception("%s\nDon't know how to create this type of object." % lls)
